...
ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$MASTER 'bash -s' < master.sh
...
ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$MASTER 'bash -s' < master.sh
# Get Token
TOKEN=$(ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$MASTER 'sudo kubeadm token create --print-join-command')
printf "\n\n"
echo $TOKEN
printf "\n\n"
...
ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$MASTER 'bash -s' < k8s_config_network_weave.sh
...
for N in $(seq 1 $WORKER_NODES); do
    ...
    ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$NODE 'bash -s' < workers.sh
done

echo "   VERIFICANDO NODES NO MASTER :"
printf "\n\n"
ssh -oStrictHostKeyChecking=no -i ~/environment/labsuser.pem ubuntu@$MASTER 'kubectl get nodes'
